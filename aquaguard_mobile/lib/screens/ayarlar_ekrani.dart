...
class _AyarlarEkraniState extends State<AyarlarEkrani> {
  ...
  void initState() {
    ...
    _hostController = TextEditingController(text: durum.mqttHost);
    _portController = TextEditingController(text: durum.mqttPort.toString());
  }
        ...
        children: [
                ...
                child: Column(
                  children: [
                    TextFormField(
                      ...
                      validator: (deger) =>
                          (deger == null || deger.trim().isEmpty) ? 'Broker adresi gerekli' : null,
                    ),
                    ...
                    TextFormField(
                      ...
                      validator: (deger) {
                        ...
                        if (sayi == null || sayi <= 0 || sayi > 65535) return 'Geçerli bir port girin';
                        return null;
                      },
                    ),
                    const SizedBox(height: 12),
                    Align(
                      ...
                      child: FilledButton.icon(
                        onPressed: () {
                          if (!(_formAnahtari.currentState?.validate() ?? false)) return;
                          context.read<UygulamaDurumu>().mqttAyarlariniGuncelle(
                                host: _hostController.text.trim(),
                                port: int.parse(_portController.text.trim()),
                              );
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('Ayarlar kaydedildi, yeniden bağlanılıyor...')),
                          );
                        },
          ...
          Card(
            child: SwitchListTile(
              ...
              subtitle: const Text('Durum değiştiğinde uygulama içi bildirim göster'),
              value: durum.bildirimlerAcik,
              onChanged: (deger) => context.read<UygulamaDurumu>().bildirimleriDegistir(deger),
            ),
          ),
        ],
      ),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Text(metin, style: TextStyle(color: renk, fontWeight: FontWeight.w600)),
        ],
      ),
    );
  }
