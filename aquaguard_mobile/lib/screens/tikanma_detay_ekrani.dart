...
class TikanmaDetayEkrani extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final sonOkumalar = durum.gecmis(zonNumarasi).take(20).toList().reversed.toList();
...
class _DurumOzetKarti extends StatelessWidget {
  ...
  const _DurumOzetKarti({required this.okuma, required this.cevrimici, required this.renk});
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(DurumRenkleri.ikonGetir(okuma: okuma, cevrimici: cevrimici), color: renk, size: 32),
            ...
            _bilgiSatiri('Son Güncelleme', DateFormat('dd.MM.yyyy HH:mm:ss').format(okuma.zaman)),
          ],
        ),
      ),
    );
...
class _TedaviBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: ListTile(
        leading: const Icon(Icons.build_circle, color: DurumRenkleri.tedaviAktif),
        ...
        onTap: () => Navigator.of(context).push(
          MaterialPageRoute(builder: (_) => AktifTedaviEkrani(zonNumarasi: zonNumarasi)),
        ),
      ),
    );
  }
...
class _SensorTrendIzgarasi extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final tanimlar = <(String baslik, String birim, Color renk, double Function(SensorOkuma) secici)>[
      ...
      ('Diferansiyel Basınç', 'bar', const Color(0xFFC62828), (o) => o.deltaBasinc),
    ];
