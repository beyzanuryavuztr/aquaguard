...
void main() {
  ...
  testWidgets('tasma/istisna olmadan cizilir, tum bilgi kartlari gorunur', (
    ...
    expect(find.text('İş Fizibilitesi'), findsOneWidget);
  ...
  testWidgets('İş Fizibilitesi satırına dokununca ekran açılır', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(500, 1400));
    addTearDown(() => tester.binding.setSurfaceSize(null));
    ...
    await tester.tap(find.text('İş Fizibilitesi'));
    await tester.pumpAndSettle();

    expect(tester.takeException(), isNull);
    expect(find.byType(IsFizibilitesiEkrani), findsOneWidget);
  });
  ...
  testWidgets('Ayarlar ekranindaki Hakkinda satiri HakkindaEkrani\'na gecer', (
    ...
    // "Hakkında" artik "Diğer" katlanir kategorisinin icinde -- once o
    // kategoriyi ac (bkz. ayarlar_ekrani_widget_test.dart ayni not,
    // 2026-09-25 kategori gruplandirmasi).
