...
|---|---|---|---|---|
| Y1 | **ADC voltaj uyumsuzluğu.** ESP32 ADC en çok 3,3 V okur; kod basınç için 0,5–4,5 V, türbidite için ~4,2 V varsayıyor, **bölücü oranı sabiti yok**. Pinler ve kalibrasyon sabitleri "yer tutucu"; hiç donanım testi yok. | `config.h` (ADC_REFERANS_VOLTAJ, BASINC_*, TURBIDITE_*) | Gerçek modül çıkışlarını ölç, bölücü + firmware'de geri ölçekleme ekle; kalibrasyon yap. | **DÜZELTİLDİ** kodda (bölücü oranı + derleme zamanı kontrol); **kalibrasyon donanımla yapılmalı** |
| Y2 | **Veri saklama sınırı ile "7g/30g" görünümleri çelişiyor.** Sınır: zon başına **10.000 satır veya 7 gün**. Gerçek cihaz 10 sn'de bir yayınlar → 10.000 satır ≈ **28 saat**; demo (1,5 sn) ≈ 4 saat. "30g" seçeneği 7 günlük saklamayla zaten ulaşılamaz. Tedavi sayaçları, başarı oranı ve maliyet özeti bu kısa pencereden hesaplanıyor → **yanıltıcı olabilir**. | `drift_sensor_okuma_repository.dart`, `sensor_trend_grafigi.dart:29` (TrendDonemi) | Kademeli saklama (örn. dakikalık özet), ya da UI'de "mevcut veri aralığı" göster ve 30g'yi kaldır. | **DÜZELTİLDİ** |
| Y3 | **Vana durumu cihazdan raporlanmıyor.** MQTT JSON'da vana alanı yok; uygulama vana durumunu kendi tahminiyle tutuyor (K1 ile birleşince uyumsuzluk). | `mqtt_handler.h` JSON alanları, `sensor_okuma.dart` | JSON şemasına `ana_vana_acik` ekle (3 yerde senkron). | **DÜZELTİLDİ** |
| Y4 | **GitHub Pages (https) + varsayılan `ws://` (8080):** tarayıcı karışık içeriği engeller, gerçek mod Pages'te ancak TLS (WSS 8081) açıksa çalışır. | `mqtt_istemci_web.dart`, `ayarlar_sabitleri.dart` | Web'de TLS'i zorunlu kıl/uyar. | **DÜZELTİLDİ** (uyarı; TLS zorlanmıyor) |
| Y5 | Release imzası **debug anahtarıyla**; `POST_NOTIFICATIONS`/`USE_BIOMETRIC` izinlerinin manifestte olup olmadığı doğrulanmadı (Android 13+ bildirim çalışmayabilir). | `android/app/build.gradle.kts:32` | Gerçek keystore; izinleri kontrol et. | **DÜZELTİLDİ** (izinler eklendi, `key.properties` ile imza; Android SDK yok, derlenmedi; gerçek keystore kullanıcıda) |
| K6 | **PubSubClient varsayılan 256 bayt paket sınırı:** telemetri JSON'u bundan büyüktür; `publish` sessizce `false` dönüp veriyi düşürürdü (cihaz "bağlı" ama uygulamaya veri gelmezdi). | `mqtt_handler.h` | `setBufferSize(768)`. | **DÜZELTİLDİ** (donanımda denenmedi) |
| K7 | **Gerçek modda sentetik/demo veri sızıntısı:** demo ve gerçek veri aynı veritabanını paylaşıyordu; geçmiş/sayaçlar karışıyordu. | `uygulama_durumu.dart` | İki ayrı veritabanı + mod ayrımı. | **DÜZELTİLDİ** |

## ORTA

| # | Bulgu | Kanıt / Not | Öneri | Durum |
|---|---|---|---|---|
| O1 | **Model Performansı ekranında sabit "%89.9 ± %2.3".** Modeli yeniden eğitince 5-fold CV **%89.9 ± %1.5**, test seti %90.8 çıkıyor (ortalama tutuyor, std tutmuyor). Görseller statik PNG. | `model_performansi_ekrani.dart:37` | Sayıyı güncel çıktıyla eşitle (ya da PNG'yi yeniden üret). | **DÜZELTİLDİ** |
...
| O3 | Sıcaklık sensörü yok (bilinçli); pH/EC ölçümleri sıcaklık telafisiz → doğruluk sınırı. | `PROJE_BRIEF.md:50`, `config.h` EC notu | Belgeye sınırlama olarak yaz. | **DÜZELTİLDİ** (README'de sınırlama) |
| O4 | CI firmware'i derlemiyor. Artık derlenebiliyor (arduino-cli, genel ESP32). | `.github/workflows/ci.yml` | Genel ESP32 derleme işi ekle (Deneyap tanımı ayrı doğrulanır). | **DÜZELTİLDİ** (CI işi eklendi; ilk GitHub çalıştırması bekleniyor) |
...
| O6 | Depo kökünde rastgele adlı 6,7 MB PDF: `BwL3XfOWsM5o7zUlVY8gJxe6H11gJvso (3).pdf` (ilk commit). | `git ls-files` | İçeriği kontrol et; anlamlı ada taşı ya da kaldır. | **DÜZELTİLDİ** (`docs/teknofest_sunum_sablonu.pdf` olarak taşındı) |
...
- **Firmware derleniyor** (genel ESP32, arduino-cli; %31 flash, %7 RAM, ArduinoJson 7 kaynaklı kullanımdan-kalkma uyarıları var; hata yok).
- **Testler:** Flutter 415, Python 44; `flutter analyze --fatal-infos` temiz; web build başarılı; skip'li test ve TODO/FIXME yok.
